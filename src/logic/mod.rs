...
pub fn check_combination(mut tiles: Query<(&mut Tile, &Transform)>, mut score: ResMut<Score>) {
    ...
    clear_tiles(&mut tiles, &tiles_to_clear);
}
...
fn clear_tiles(tiles: &mut Query<(&mut Tile, &Transform)>, tiles_to_clear: &[(usize, usize)]) {
    let half_map_size = (MAP_SIZE as f32 * TILE_SIZE) / 2.0;
